...
export const useSVM = () => {
  ...
  const sendSplBatchTokenTransaction = async (
    ...
    const maxInstructionsPerTx = 10; // Reduced to prevent transaction failures
    ...
    for (let i = 0; i < recipients.length; i += maxInstructionsPerTx) {
      ...
      for (const { address, amount } of batch) {
        try {
          ...
          const recipientATAAccount = await connection.getAccountInfo(
            recipientATA
          );

          if (!recipientATAAccount) {
            console.log(
              `Recipient ATA does not exist for ${address}, skipping...`
            );
            continue; // Skip this recipient if ATA doesn't exist
          }
